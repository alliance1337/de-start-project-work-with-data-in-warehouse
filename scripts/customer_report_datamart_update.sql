...
dwh_update_delta AS ( -- делаем выборку, по которым были изменения в DWH. По этим заказчикам данные в витрине нужно будет обновить
    SELECT     
                        ...
                        *,
                        ...
                        FROM ( 
                            SELECT -- в этой выборке делаем расчёт по большинству столбцов, так как все они требуют одной и той же группировки, кроме столбца с самой популярной категорией товаров у мастера. Для этого столбца сделаем отдельную выборку с другой группировкой и выполним JOIN
                                ...
                                INNER JOIN (
                                    SELECT     -- самая популярная категория товаров у этого заказчика за месяц. 
                                ...
                                INNER JOIN (
                                    SELECT     -- идентификатор самого популярного мастера ручной работы у заказчика
                                            ...
                                            FROM dwh_delta AS dd
                                                GROUP BY dd.customer_id, dd.craftsman_id
                                                    ORDER BY count_craftsman DESC) AS T5 ON T2.customer_id = T5.customer_id_for_craftsman_id
                ) AS T4 WHERE T4.rank_count_product = 1 and rank_count_craftsman = 1 ORDER BY report_period -- условие помогает оставить в выборке первую по популярности категорию товаров
),
                                ...
                                INNER JOIN (
                                    SELECT     -- идентификатор самого популярного мастера ручной работы у заказчика
                                            ...
                                            FROM dwh_delta AS dd
                                                GROUP BY dd.customer_id, dd.craftsman_id
                                                    ORDER BY count_craftsman DESC) AS T5 ON T2.customer_id = T5.customer_id_for_craftsman_id
                ) AS T4 WHERE T4.rank_count_product = 1 and rank_count_craftsman = 1 ORDER BY report_period
),
